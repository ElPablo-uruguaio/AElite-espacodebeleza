    ...
    <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 shadow-xl">
      ...
      <div className="flex gap-2 mb-6">
        ...
        <button 
          ...
          className="bg-red-600 hover:bg-red-500 text-white px-4 py-2 rounded-xl flex items-center gap-1 font-bold text-sm"
        >
      ...
      <div className="space-y-2">
        {blockedClients.map(b => (
          <div key={b.id} className="flex justify-between items-center bg-zinc-950 p-4 rounded-xl border border-zinc-800">
            <div>
              <p className="text-sm font-bold text-white">{b.cliente_nome} <span className="text-zinc-500 text-xs font-normal">({b.cliente_phone})</span></p>
              <p className="text-xs text-zinc-500">{b.motivo}</p>
            </div>
