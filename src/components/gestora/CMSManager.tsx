...
import { Settings, Plus, Trash2, Edit3, Image, Film, Key, CheckCircle2, Lock, Scissors, Sparkles, Star } from 'lucide-react';
import { useSalon } from '../../context/SalonContext';
import { useAuth } from '../../context/AuthContext';

export const CMSManager: React.FC = () => {
  const { settings, updateSettings, services, addService, deleteService, stories, addStory, deleteStory } = useSalon();
      ...
      {activeTab === 'settings' && (
        <form onSubmit={handleSaveSettings} className="space-y-4 max-w-2xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-zinc-300 mb-1">Chave PIX (Para Sinal R$ 20)</label>
              <input
                type="text"
                value={chavePix}
          ...
          <div>
            <label className="block text-xs font-semibold text-zinc-300 mb-1">URL da Imagem de Banner da Home</label>
            <input
              type="url"
              value={bannerInput}
      ...
      {activeTab === 'services' && (
        <div className="space-y-6">
          <form onSubmit={handleAddService} className="bg-zinc-950 border border-zinc-800 p-5 rounded-2xl space-y-4">
            ...
            <div>
              <input
                ...
                placeholder="URL da Foto do Serviço (https://...)"
                className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-3 py-2 text-xs text-white"
              />
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {services.map(s => (
              <div key={s.id} className="bg-zinc-950 border border-zinc-800 p-4 rounded-2xl flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <img src={s.foto_url} alt={s.nome} className="w-12 h-12 rounded-xl object-cover" />
                  <div>
                    <h5 className="text-sm font-bold text-white">{s.nome}</h5>
